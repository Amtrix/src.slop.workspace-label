...
`opt_component_feature_timer` adds a small countdown-timer box under the overlay. Each timer shows the workspace it was created on as a `[N]` prefix, then its name and remaining time as `HH:MM:SS`, with pause/resume, restart, and remove (`✕`) buttons on the row; an **ADD** button opens a dialog asking for a name and a duration, and a **CLEAR** button removes all timers after a confirmation prompt. Pausing freezes the countdown (and stays frozen across an app restart) until you resume it, and restart resets the timer back to its full original duration. When a timer reaches `00:00:00` its time turns red and holds at zero, and the label of the workspace the timer was set on starts flashing as an alert. The flashing stops as soon as you click that label (even if you are already on that workspace). It takes two optional properties:
